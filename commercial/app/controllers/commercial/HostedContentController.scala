...
class HostedContentController(contentApiClient: ContentApiClient)
  ...
  def renderOnwardComponent(campaignName: String, pageName: String, trailCount: Int) = Action.async {
    implicit request =>
      ...
      capiResponse map { response =>
        response.results map { results =>
          val trails = HostedTrails.fromContent(s"advertiser-content/$campaignName/$pageName", trailCount, results)
          def cached(html: Html) = Cached(cacheDuration)(JsonComponent(html))
          trails.headOption map { trail =>
            cached(hostedVideoOnwardComponent(trail))
          } getOrElse {
